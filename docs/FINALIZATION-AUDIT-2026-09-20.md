...
| F-013 | Post-merge CI | Main-branch `Build firmware from ZIP` copied the package but not root `src`, causing `cp: cannot stat .../src/*`. | Copy root `src` into the OpenWrt tree, trigger on `src/**`, assert daemon sources, and emit TEST-only traceability files. | RESOLVED; REBUILDING IN FIX PR |
