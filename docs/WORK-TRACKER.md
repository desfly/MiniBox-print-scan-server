...
- [x] Reproduced the failure read-only on the physical MiniBox without installing or flashing anything: `uhttpd`, printerd, scand and discoveryd are running and boot-enabled, but `/www` is empty and `/www/cgi-bin` is absent. Port 80 returns only the directory index; 631/health and 8080/health respond, eSCL is Idle, and RAW 9100 remains closed.
- [x] Recorded the live resource baseline: 55,676 KiB RAM total, 15,876 KiB available at idle, approximately 9.0 MiB free overlay. Full LuCI remains an unjustified cost for this 16 MiB / 64 MiB device while the minimal integrated UI meets the current requirement.
- [x] Read-only hardware layout check: the real `firmware` MTD partition is 16,580,608 bytes, exactly matching the CI size gate; `u-boot` (131,072 bytes) and final `art` (65,536 bytes) are separate. No MTD contents were read or written. Kernel console configuration mentions `ttyATH0,115200`, but physical UART recovery remains unverified/nonfunctional and this is not flash approval.
