...
static void check_wire_attributes(const unsigned char*b,size_t n){
    ...
    assert(tag==0x45);
    assert(find_attribute(b,n,"printer-uuid",&tag,&v,&len));
    assert(tag==0x45&&len==strlen("urn:uuid:4d424f58-0000-4000-8000-0cefafcfc53d"));
    assert(!memcmp(v,"urn:uuid:4d424f58-0000-4000-8000-0cefafcfc53d",len));
    assert(find_attribute(b,n,"printer-device-id",&tag,&v,&len));
    assert(tag==0x41&&len==strlen("MFG:Hewlett-Packard;MDL:HP LaserJet M1522n MFP;CMD:PCLXL,PCL,POSTSCRIPT;CLS:PRINTER;"));
    assert(!memcmp(v,"MFG:Hewlett-Packard;MDL:HP LaserJet M1522n MFP;CMD:PCLXL,PCL,POSTSCRIPT;CLS:PRINTER;",len));
...
int main(void){const unsigned char q[]={2,0,0,0x0b,0,0,0,7};const unsigned char print_op[]={0,0,0,2};const unsigned char validate_op[]={0,0,0,4};const unsigned char create_op[]={0,0,0,5};const unsigned char send_op[]={0,0,0,6};const unsigned char cancel_op[]={0,0,0,8};const unsigned char job_attrs_op[]={0,0,0,9};const unsigned char jobs_op[]={0,0,0,10};const unsigned char attrs_op[]={0,0,0,0x0b};struct ipp_request r;unsigned char z[2048];size_t n;assert(ipp_parse_header(q,sizeof q,&r)==0);assert(r.major==2&&r.minor==0&&r.operation==IPP_OP_GET_PRINTER_ATTRIBUTES&&r.request_id==7);assert(!strcmp(ipp_operation_name(IPP_OP_PRINT_JOB),"Print-Job"));assert(ipp_parse_header(q,7,&r)==-1);n=ipp_build_status(z,sizeof z,&r,0);assert(n>9);assert(z[0]==2&&z[1]==0&&z[2]==0&&z[3]==0&&z[7]==7&&z[8]==1&&z[n-1]==3);n=ipp_build_printer_attributes(z,sizeof z,&r,"ipp://minibox.local/ipp/print");assert(n>9);assert(z[0]==2&&z[1]==0&&z[7]==7);assert(z[8]==0x01);assert(z[n-1]==0x03);check_wire_attributes(z,n);assert(contains(z,n,"printer-name"));assert(contains(z,n,"HP LaserJet M1522n @ MiniBox"));assert(contains(z,n,"printer-uri-supported"));assert(contains(z,n,"ipp://minibox.local/ipp/print"));assert(contains(z,n,"operations-supported"));assert(contains_bytes(z,n,print_op,sizeof print_op));assert(contains_bytes(z,n,validate_op,sizeof validate_op));assert(contains_bytes(z,n,create_op,sizeof create_op));assert(contains_bytes(z,n,send_op,sizeof send_op));assert(contains_bytes(z,n,cancel_op,sizeof cancel_op));assert(contains_bytes(z,n,job_attrs_op,sizeof job_attrs_op));assert(contains_bytes(z,n,jobs_op,sizeof jobs_op));assert(contains_bytes(z,n,attrs_op,sizeof attrs_op));assert(contains(z,n,"job-creation-attributes-supported"));assert(contains(z,n,"document-format-supported"));assert(attribute_has_exact_value(z,n,"printer-name","HP LaserJet M1522n @ MiniBox"));assert(attribute_has_exact_value(z,n,"printer-make-and-model","HP LaserJet M1522n"));assert(attribute_has_exact_value(z,n,"printer-info","MiniBox network print server"));assert(attribute_has_exact_value(z,n,"printer-uuid","urn:uuid:4d424f58-0000-4000-8000-0cefafcfc53d"));assert(attribute_has_exact_value(z,n,"printer-device-id","MFG:Hewlett-Packard;MDL:HP LaserJet M1522n MFP;CMD:PCLXL,PCL,POSTSCRIPT;CLS:PRINTER;"));assert(attribute_has_exact_value(z,n,"document-format-supported","application/octet-stream"));
