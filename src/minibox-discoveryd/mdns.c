...
static int ipv4_for_interface(const char *ifname,struct in_addr *addr){
    struct ifaddrs *all,*it;int found=0;
...
int mb_mdns_serve(const mb_service_t *services,size_t count,const char *hostname,
    ...
    int fd,one=1;struct sockaddr_in bindaddr,dst;struct ip_mreq membership;
    ...
    while(!*stop){
        ...
        if(ready>0&&FD_ISSET(fd,&fds)){
            struct sockaddr_in peer;socklen_t peerlen=sizeof peer;struct in_addr ip;
            ssize_t n=recvfrom(fd,in,sizeof in,0,(struct sockaddr*)&peer,&peerlen);
            if(n>0&&ipv4_for_interface(NULL,&ip)==0){
                int m=answer_packet(out,sizeof out,services,count,hostname,in,(size_t)n,ip);
